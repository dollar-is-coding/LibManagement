    ...
    @else
        @foreach ($sach->hasGioSach as $key_gs => $gio_sach)
            @if ($gio_sach->doc_gia_id == Auth::id())
                <a id="sach_{{ $sach->id }}" onclick="handleGioSach({{ $sach->id }})" class="btn danger-btn">
                    Bỏ chọn</a>
            @break
        @endif
        @if ($gio_sach->hasGioSach->count() - 1 == $key_gs)
            <a class="btn custom-btn" id="sach_{{ $sach->id }}" onclick="handleGioSach({{ $sach->id }})">
                Chọn sách</a>
        @endif
    @endforeach
@endif
...
@if ($sach->hasPhieuMuon->count() - 1 == $pm_key)
    @if ($sach->hasGioSach->count() == 0)
        <a class="btn custom-btn" id="sach_{{ $sach->id }}"
            onclick="handleGioSach({{ $sach->id }})">Chọn sách</a>
    @else
        @foreach ($sach->hasGioSach as $key_gs => $gio_sach)
            @if ($gio_sach->doc_gia_id == Auth::id())
                <a id="sach_{{ $sach->id }}" onclick="handleGioSach({{ $sach->id }})"
